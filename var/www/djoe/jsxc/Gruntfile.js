...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    copy : {
      ...
      init : {
        files : [{
          expand : false, src : 'src/jsxc_init.js', dest : '<%= target %>/jsxc_init.js'
        }]
      },
    ...
    concat : {
      ...
      dep : {
        options : {
          ...
          ' */\n\n', process : function(src, filepath) {
            ...
            if (data) {
              return '\n/*!\n * Source: ' + filepath + ', license: ' + data.license + ', url: ' +
                  data.url + '\n */\n' + src;
            } else {
              return src;
            }
      ...
      jsxc : {
        ...
        src : ['src/jsxc.intro.js', 'src/jsxc.lib.js', 'src/jsxc.lib.xmpp.js',
          'src/jsxc.lib.mmstream.js', 'src/jsxc.lib.mmstream.gui.js', 'src/jsxc.lib.gui.js',
          'src/jsxc.lib.newgui.js', 'src/jsxc.lib.*.js', 'tmp/template.js', 'src/jsxc.outro.js'],
    ...
    watch : {
      locales : {
        ...
        files : ['scss/*'], tasks : ['sass', 'autoprefixer', 'replace:imageUrl', 'concat_css']
      }, js : {
        files : ['src/jsxc.lib.*'], tasks : ['concat:jsxc']
      }, copy : {
        files : ['src/jsxc_init.js'], tasks : ['copy:init']
      }, template : {
        files : ['template/*.html'], tasks : ['htmlConvert', 'replace:template', 'concat:jsxc']
      }, webpack : {
    ...
    concat_css : {
      options : {
        ...
        src : depcss, dest : "<%= target %>/css/jsxc.css"
      },
    },

    htmlConvert : {
      options : {
        target : 'js', rename : function(name) {
    ...
    webpack : {
      jsxc : {
        // webpack options
        entry : "./tmp/jsxc.js", output : {
          path : "<%= target %>/", filename : "jsxc.js", overwrite : true
        },
    ...
    shell : {
      'precommit-before' : {
        command : 'git diff --cached --name-only', options : {
...
// These plugins provide necessary tasks.
  ...
  grunt.registerTask('build',
      ['jshint', 'clean', 'sass', 'replace:imageUrl', 'autoprefixer', 'copy', 'merge_data',
        'replace:locales', 'htmlConvert', 'replace:template', 'concat', 'webpack:jsxc',
        'concat_css']);
  ...
  grunt.registerTask('build:prerelease', 'Build a new pre-release', function() {
    ...
    grunt.task.run(
        ['build', 'usebanner', 'replace:version', 'replace:libraries', 'replace:todo', 'uglify']);
  });
  ...
  grunt.registerTask('commit',
      ['shell:precommit-before', 'search:console', 'jsbeautifier', 'prettysass', 'jshint',
        'shell:precommit-after']);
};
