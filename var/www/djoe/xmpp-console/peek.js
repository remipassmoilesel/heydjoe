...
$(document).ready(function() {
   ...
   */
  $('#login_dialog').dialog({
    autoOpen : true, draggable : false, modal : true, title : 'Connect to XMPP', buttons : {
      "Connect" : function() {
  ...
  /**
   * Lors de l'appui sur le bouton envoyer, parser le conteneu du champs texte puis le transmettre
   * au serveur XMPP
   */
...
$(document).bind('connect', function(ev, data) {
  ...
  conn.connect(data.jid, data.password, function(status) {
    if (status === Strophe.Status.CONNECTED) {
      ...
      $("#statusBar").text("Connecté. JID: " + Peek.connection.jid);
    } else if (status === Strophe.Status.DISCONNECTED) {
      $(document).trigger('disconnected');
      $("#statusBar").text("Déconnecté");
    }
  });
